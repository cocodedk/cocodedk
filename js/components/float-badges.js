...
function getPhoneLabel(lang) {
  return phoneLabels[lang] || phoneLabels.da;
}
...
function initFloatBadges() {
  ...
  const initialLang = (document.documentElement.lang || 'da').toLowerCase();
  const label = getPhoneLabel(initialLang);
  const container = document.createElement('div');
  container.id = 'float-badges';
  container.innerHTML = `
    <button type="button" class="float-badge float-badge--phone" onclick="window.floatBadges.showPhone()" title="${label}" aria-label="${label}">
...
window.floatBadges = {
  ...
  updateLanguage(lang) {
    const btn = document.querySelector('.float-badge--phone');
    if (!btn) return;
    const label = getPhoneLabel((lang || '').toLowerCase());
    btn.title = label;
